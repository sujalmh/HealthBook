-- HealthBook migration: Supabase as source of truth (server-truth).
...
CREATE OR REPLACE FUNCTION public.healthbook_my_patient_ids()
RETURNS SETOF text
...
CREATE OR REPLACE FUNCTION public.healthbook_visible_patient_ids()
RETURNS SETOF text
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT p.patient_id FROM public.profiles p WHERE p.auth_user_id = auth.uid()
  ...
  WHERE COALESCE(cc.status, cc.payload ->> 'status') = 'active'
    AND (
      cc.payload ->> 'doctorId' IN (SELECT public.healthbook_my_patient_ids())
      OR cc.payload ->> 'doctorUserId' IN (SELECT public.healthbook_my_patient_ids())
    );
$$;

CREATE OR REPLACE FUNCTION public.healthbook_writable_patient_ids()
RETURNS SETOF text
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT p.patient_id FROM public.profiles p WHERE p.auth_user_id = auth.uid()
...
CREATE POLICY carecircle_insert_own ON care_circle FOR INSERT
  WITH CHECK (patient_id IN (SELECT public.healthbook_my_patient_ids()));
DROP POLICY IF EXISTS carecircle_update_own ON care_circle;
CREATE POLICY carecircle_update_own ON care_circle FOR UPDATE
  USING (
    patient_id IN (SELECT public.healthbook_my_patient_ids())
    OR payload ->> 'doctorId' IN (SELECT public.healthbook_my_patient_ids())
    OR payload ->> 'doctorUserId' IN (SELECT public.healthbook_my_patient_ids())
  )
  WITH CHECK (
    patient_id IN (SELECT public.healthbook_my_patient_ids())
    OR payload ->> 'doctorId' IN (SELECT public.healthbook_my_patient_ids())
...
CREATE POLICY grants_select ON doctor_grants FOR SELECT
  USING (
    patient_id IN (SELECT public.healthbook_visible_patient_ids())
...
CREATE POLICY grants_update_own ON doctor_grants FOR UPDATE
  USING (patient_id IN (SELECT public.healthbook_my_patient_ids()))
  WITH CHECK (patient_id IN (SELECT public.healthbook_my_patient_ids()));
