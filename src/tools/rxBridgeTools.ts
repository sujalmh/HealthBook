...
export const suggestQuestionForDoctorTool: WebMCPToolDefinition = {
  ...
  execute: async (
    ...
    try {
      ...
      const systemPrompt = `You are a clinical care coordinator. Given patient context, medication name, actual discharge medication list, recent labs, and optional document image/text, generate a single targeted question for the doctor that is specific, actionable, and grounded. Consider therapeutic indication, organ system monitoring requirements, adverse effect surveillance, dose titrations, and pertinent laboratory correlations. Return ONLY valid JSON with shape {"questionText": string, "confidence": number, "reasoning": string}. Provide confidence 0-1 and grounded reasoning. Use vision+text together when image provided. No markdown.`;
