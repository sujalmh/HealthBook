...
async function aiCorrelateNarrative(
  ...
  try {
    ...
    const systemPrompt = `You are a clinical causal biomarker assistant. Given longitudinal lab data and medications, generate a concise causal narrative. Use Exa highlights when provided for authoritative reference. Return ONLY valid JSON with shape {"trajectory": string, "causalStorySentence": string, "recommendedDoctorQuestion": string, "correlatedMedications": string[], "confidenceScore": number}. Identify the physiological trajectory pattern (e.g. rising, declining, improving, elevated, shifted, or stable) relative to reference thresholds. The causal story should reference the direction of biomarker movement and correlate with ongoing or adjusted pharmacotherapy plainly for patient comprehension. No markdown.`;
