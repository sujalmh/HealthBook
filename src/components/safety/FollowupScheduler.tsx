      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-100">Order Direct Clinical Follow-Up</h3>
              <p className="text-xs text-slate-400">Prescribe urgent in-person or telehealth review</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-300">Appointment Mode</label>
          ...
          {/* Timing / Offset */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-300">Target Timing</label>
            ...
            {dateOffset === 'custom' && (
              <input
                ...
                className="w-full mt-2 bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-200"
              />
            )}
          </div>
          ...
          {/* Provider Name */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-300">Attending Clinician / Specialty</label>
            <input
              type="text"
              value={providerName}
          ...
          {/* Clinical Reason */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-300">Clinical Purpose & Evaluation Goal</label>
            <textarea
              rows={2}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-xs text-slate-200 leading-relaxed"
            />
          </div>
          ...
          <div className="bg-slate-950/60 rounded-2xl p-3 border border-slate-800 flex items-center justify-between text-xs text-slate-400">
