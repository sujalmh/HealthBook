...
export const WebMCPInspector: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-5xl h-[85vh] shadow-2xl flex flex-col text-slate-100 overflow-hidden">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-100">CareCanvas WebMCP Inspector</h2>
                <span className="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider bg-emerald-500/20 text-emerald-400 border border-emerald-500/40">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === 'catalog' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="text-xs text-slate-400 font-mono">Showing {filteredTools.length} tools</div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredTools.map((tool) => (
                  <div
                    key={tool.name}
                    className="bg-slate-950/70 border border-slate-800 hover:border-slate-700 rounded-xl p-4 space-y-3"
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="font-mono text-sm font-bold text-sky-300">{tool.name}</div>
                    ...
                    <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between text-xs">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-sky-400 hover:text-sky-300 font-semibold text-xs"
                      >
          ...
          {activeTab === 'telemetry' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="text-xs text-slate-400">Real-Time WebMCP Dispatcher Stream</div>
                <button
              ...
              {telemetryLogs.length === 0 ? (
                ...
                <div className="space-y-3">
                  {telemetryLogs.map((log) => (
                    <div
                      key={log.id}
                      className="bg-slate-950/70 border border-slate-800 rounded-xl p-4 space-y-2 text-xs font-mono"
                    >
                      <div className="flex items-center justify-between font-sans">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold font-mono text-sky-300 text-sm">{log.toolName}</span>
                      ...
                      {log.result?.plainLanguageExplanation && (
                        <p className="font-sans text-xs text-slate-300 leading-relaxed bg-slate-900/60 p-2.5 rounded-lg border border-slate-800">
                          {log.result.plainLanguageExplanation}
                        </p>
                      )}
                      ...
                      {expandedLogId === log.id && (
                        <div className="grid grid-cols-2 gap-3 pt-2 text-[11px]">
                          <div>
                            <div className="text-slate-400 mb-1 font-bold">Input Params:</div>
                            <pre className="bg-slate-900 p-2 rounded-lg border border-slate-800 overflow-x-auto text-emerald-300">
                              {JSON.stringify(log.params, null, 2)}
                            </pre>
                          </div>
                          <div>
                            <div className="text-slate-400 mb-1 font-bold">Tool Result:</div>
                            <pre className="bg-slate-900 p-2 rounded-lg border border-slate-800 overflow-x-auto text-sky-300">
          ...
          {activeTab === 'playground' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-sm text-slate-100 font-mono focus:border-sky-500 focus:outline-none"
                  >
                    {tools.map((t) => (
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] text-slate-400 font-mono">JSON Schema Draft 2020-12</span>
                  </div>
                  <textarea
                    value={playgroundParams}
                    onChange={(e) => setPlaygroundParams(e.target.value)}
                    rows={12}
                    className="w-full p-3 bg-slate-950 border border-slate-700 rounded-xl font-mono text-xs text-emerald-400 focus:border-sky-500 focus:outline-none leading-relaxed"
                  />
                </div>
              ...
              <div className="space-y-4">
                <div className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Execution Output</div>

                {playgroundResult ? (
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-3 font-mono text-xs overflow-auto max-h-[460px]">
                    <div className="flex items-center justify-between pb-2 border-b border-slate-800 font-sans">
                      ...
                      <span className="text-[10px] text-slate-500">{playgroundResult.timestamp}</span>
                    </div>

                    {playgroundResult.plainLanguageExplanation && (
                      <div className="bg-slate-900 p-3 rounded-lg border border-slate-800 text-slate-200 font-sans text-xs leading-relaxed">
                        <strong>Plain Language Narrative:</strong>
                        <p className="mt-1 text-slate-300">{playgroundResult.plainLanguageExplanation}</p>
                      </div>
                    )}
                  ...
                  <div className="h-[400px] flex flex-col items-center justify-center p-8 text-center bg-slate-950/40 rounded-xl border border-dashed border-slate-800 text-slate-500 text-xs">
              ...
              {pendingApprovals.length === 0 ? (
                ...
                <div className="space-y-3">
                  {pendingApprovals.map((item) => (
                    <div
                      key={item.id}
                      className="bg-slate-950 border border-amber-500/40 rounded-xl p-4 flex items-center justify-between gap-4"
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-rose-300 rounded-lg text-xs font-medium border border-slate-700 transition-colors"
                        >
