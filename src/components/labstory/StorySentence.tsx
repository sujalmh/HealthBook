...
export const StorySentence: React.FC<StorySentenceProps> = ({ marker, labs, className = '' }) => {
  if (!labs || labs.length === 0) {
    return (
      <div className={`bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex items-center gap-3 ${className}`}>
        <Sparkles className="w-5 h-5 text-sky-400 shrink-0" />
        <p className="text-xs text-slate-400">
          No longitudinal data points available for <span className="font-semibold text-slate-200">{marker}</span>. Upload a lab report or select another marker.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-gradient-to-r from-slate-900 via-slate-900/95 to-slate-900 border border-slate-800 rounded-2xl p-4 shadow-md ${className}`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {/* Quick Delta Stats */}
        <div className="flex items-center gap-2 sm:self-center shrink-0 border-t sm:border-t-0 sm:border-l border-slate-800 pt-2 sm:pt-0 sm:pl-4">
          <div className="text-right">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Trajectory Delta</div>
            <div className={`text-xs sm:text-sm font-black ${delta > 0 ? (isDecliningBad ? 'text-rose-400' : 'text-emerald-400') : (isImproving ? 'text-emerald-400' : 'text-slate-200')}`}>
