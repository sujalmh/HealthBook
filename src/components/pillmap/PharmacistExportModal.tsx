...
export const PharmacistExportModal: React.FC<PharmacistExportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-scale-up">
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-slate-950 text-slate-100 space-y-6 print:bg-white print:text-black">
          {/* Document Header */}
          <div className="border-b border-slate-800 pb-4 flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded bg-sky-500/20 text-sky-300 text-[10px] font-bold border border-sky-500/30 print:hidden">
          ...
          <div className="space-y-2">
            ...
            <div className="border border-slate-800 rounded-xl overflow-hidden print:border-gray-300">
              <table className="w-full text-xs text-left">
                <thead className="bg-slate-900 border-b border-slate-800 text-slate-300 font-bold uppercase text-[10px] print:bg-gray-100 print:text-black">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800/80 print:divide-gray-200">
                  {bundle.brandGenericCrosswalk.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-900/40 print:hover:bg-transparent">
                      ...
                      <td className="p-2.5 text-slate-300 print:text-gray-800">{item.generic}</td>
                      <td className="p-2.5 text-slate-400 print:text-gray-600">{item.class || 'Prescription Drug'}</td>
                      <td className="p-2.5 font-mono font-semibold text-emerald-400 print:text-black">{item.dose}</td>
                      <td className="p-2.5 text-slate-300 print:text-gray-800">{item.frequency || item.timingSlots?.join(', ') || 'Scheduled Daily'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {bundle.drugInteractions.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No drug-drug interaction conflicts flagged.</p>
                ) : (
                  bundle.drugInteractions.map((arc, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-xs space-y-1 print:bg-gray-50 print:border-gray-300"
                    >
                      ...
                      <p className="text-slate-300 text-[11px] print:text-gray-700">{arc.mechanism}</p>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {bundle.dietTimingRules.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No specific food-drug restrictions noted.</p>
                ) : (
                  bundle.dietTimingRules.map((rule, idx) => (
                    ...
                    >
                      ...
                      <p className="text-slate-300 text-[11px] print:text-gray-700">{rule.clinicalGuidance}</p>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 print:bg-gray-50 print:border-gray-300">
            ...
            <div className="text-right border-t sm:border-t-0 sm:border-l border-slate-800 pl-0 sm:pl-4 pt-2 sm:pt-0 w-full sm:w-auto">
              <div className="text-xs font-mono text-slate-300 print:text-black">
