...
export const ReconciliationWalk: React.FC<ReconciliationWalkProps> = ({
  ...
  if (!currentItem) {
    return <div className="text-center py-12 text-slate-400">No medications to reconcile.</div>;
  }
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full bg-sky-500/20 text-sky-300 font-mono font-bold text-xs border border-sky-500/40">
                Step {currentIndex + 1} of {items.length}
              </span>
              <h3 className="text-lg font-black text-white">Conversational Medication Walkthrough</h3>
            </div>
          ...
          <div className="flex items-center gap-3 bg-slate-950 px-4 py-2 rounded-2xl border border-slate-800 text-xs">
            <div className="text-right">
              ...
              <div className="text-[10px] text-slate-400 font-mono">{progressPercent}% Completed</div>
            </div>
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-inner">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-800/80 pb-5">
          <div>
            <div className="flex items-center gap-3">
              <h2 className="text-2xl font-black tracking-tight text-white">{currentItem.medName}</h2>
              <ChangeBadge status={currentItem.statusBadge} size="md" />
            </div>
            <div className="flex items-center gap-2 mt-1 text-xs text-slate-400 font-mono">
              <span>Generic Chemical: <strong className="text-slate-200">{currentItem.genericName}</strong></span>
              {currentItem.isOTC && (
          ...
          {currentItem.timingSlots && currentItem.timingSlots.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 bg-slate-900 px-3.5 py-2 rounded-2xl border border-slate-800 text-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
          ...
          <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-1.5">
            ...
            <div className="text-base font-extrabold text-slate-200">
              {currentItem.preHospDose !== 'None' ? currentItem.preHospDose : <span className="text-slate-500 italic">None</span>}
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-1.5">
            ...
            <div className="text-base font-extrabold text-sky-300">
        ...
        <div className="p-5 rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900/90 to-sky-950/30 border border-sky-900/50 shadow-lg space-y-3">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-sky-600/30 text-sky-300 flex items-center justify-center border border-sky-500/40">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Plain-language translation of why this medication changed</p>
            </div>
          </div>

          <p className="text-sm font-medium text-slate-100 leading-relaxed bg-slate-950/60 p-4 rounded-2xl border border-slate-800/80">
            {currentItem.plainLanguageExplanation}
          </p>

          {currentItem.dietInstructions && (
            <div className="flex items-center gap-2 text-xs text-amber-300 bg-amber-950/30 p-2.5 rounded-xl border border-amber-800/40 font-medium">
        ...
        {((currentItem.interactions && currentItem.interactions.length > 0) ||
          ...
          <div className="space-y-3">
            ...
            {currentItem.interactions?.map((inter) => (
              <div
                key={inter.id}
                className="p-4 rounded-2xl bg-rose-950/40 border border-rose-800/70 shadow-sm space-y-2"
              >
                ...
                <p className="text-xs text-slate-300 italic bg-slate-950/50 p-2 rounded-xl">
            ...
            {currentItem.dietInteractions?.map((diet) => (
              <div
                key={diet.id}
                className="p-3.5 rounded-2xl bg-amber-950/40 border border-amber-800/70 text-xs text-amber-200 space-y-1"
              >
                ...
                <p className="text-slate-300 italic">{diet.clinicalGuidance}</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {currentItem.suggestedQuestions.map((q, qIdx) => (
                <div
                  key={qIdx}
                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 rounded-2xl bg-slate-900 border border-slate-800 text-xs text-slate-200"
                >
                  ...
                  <button
                    ...
                    className="px-3 py-1 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 text-[11px] font-bold border border-amber-500/40 transition-colors shrink-0"
                  >
        ...
        {showNoteInput || currentItem.patientComment ? (
          <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
            ...
            <textarea
              ...
              rows={2}
              className="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500"
            />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-800">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {!showNoteInput && (
              <button
                ...
                className="px-3.5 py-2.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-semibold border border-slate-800 transition-colors"
                title="Add note"
              >
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            <button
              onClick={handleNext}
              className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold border border-slate-700 transition-colors shadow-sm"
            >
