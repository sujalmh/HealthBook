            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="text-caption text-muted font-mono mt-0.5">
                    {item.genericName !== item.medName ? `Generic: ${item.genericName}` : 'Rx Order'}
                  </div>
                </div>
                ...
                <div className="space-y-1 pt-1">
                  {item.interactions && item.interactions.length > 0 && (
                    <div className="flex items-center gap-1.5 text-xs text-rose-700 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-200">
                      ...
                      <span className="truncate">Conflict: {item.interactions[0].drugA} ↔ {item.interactions[0].drugB}</span>
                    </div>
                  )}
              ...
              <div className="grid grid-cols-1 gap-2 text-xs">
                ...
                <div className="bg-white/80 rounded-xl p-2.5 border border-slate-200 space-y-0.5">
                  ...
                  {item.inHospReason && (
                    <div className="text-[11px] text-slate-600 italic">{item.inHospReason}</div>
                  )}
                </div>
                ...
                >
                  <div className="text-[10px] font-mono font-bold uppercase opacity-80">3. Discharge Orders</div>
                  <div className="font-bold text-sm">
                    {item.dischargeDose}
                  </div>
                  {item.dischargeFrequency && (
                    <div className="text-[11px] opacity-90">{item.dischargeFrequency}</div>
                  )}
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-200/70" onClick={(e) => e.stopPropagation()}>
                <button
                  ...
                  className={`flex-1 px-4 py-2.5 rounded-xl font-bold text-xs inline-flex items-center justify-center gap-1.5 transition-all min-h-[44px] ${
                    item.isApprovedByPatient
                      ? 'bg-emerald-600 text-white shadow-sm hover:bg-emerald-700'
                      : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300'
                  }`}
                >
                  <Check className={`w-4 h-4 ${item.isApprovedByPatient ? 'text-white' : 'text-slate-600'}`} />
                  <span>{item.isApprovedByPatient ? 'Approved by Patient' : 'Approve Change'}</span>
                </button>

                <button
                  onClick={() => onSelectMed(item)}
                  className="px-3.5 py-2.5 rounded-xl bg-slate-100 hover:bg-sky-600 hover:text-white text-slate-700 border border-slate-300 transition-colors flex items-center justify-center min-h-[44px] min-w-[44px]"
                  title="Open detailed walkthrough"
                >
              ...
              filteredItems.map((item) => (
                ...
                >
                  ...
                  <td className="py-3.5 px-4 text-right space-x-1.5" onClick={(e) => e.stopPropagation()}>
                    <button
                      onClick={() => onToggleApproval(item.medId)}
                      className={`px-3 py-2 rounded-xl font-bold text-xs inline-flex items-center gap-1 transition-all min-h-[36px] ${
                        item.isApprovedByPatient
                    ...
                    <button
                      onClick={() => onSelectMed(item)}
                      className="p-2 rounded-xl bg-slate-100 hover:bg-sky-600/30 hover:text-sky-700 text-slate-600 border border-slate-200 transition-colors min-h-[36px] min-w-[36px]"
