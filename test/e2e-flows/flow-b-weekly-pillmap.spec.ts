...
export async function runFlowBTests(): Promise<{ passed: number; failed: number; errors: string[] }> {
  let passed = 0;
  let failed = 0;
  const errors: string[] = [];
  ...
  await test('Flow B E2E: Weekly PillMap Visual Polypharmacy & Adherence Simulation', async () => {
    ...
    const atorvaShift = schedRes.data.proposedShifts.find((s: any) => s.medName === 'Atorvastatin');
