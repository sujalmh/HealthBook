...
export async function runPillMapToolsTests(): Promise<{ passed: number; failed: number; errors: string[] }> {
  let passed = 0;
  let failed = 0;
  const errors: string[] = [];
  ...
  await test('TC-PM02-02: check_interactions - Orange Major arc between Apixaban and Fish Oil', async () => {
    ...
    assert(res.success);
    const arc = res.data.find((a: any) => a.drugA.includes('Apixaban') || a.drugB.includes('Apixaban'));
  ...
  await test('TC-PM03-03: check_diet_interactions - Levothyroxine empty stomach requirement', async () => {
    ...
    assert(res.success);
    const badge = res.data.find((b: any) => b.drugName === 'Levothyroxine');
  ...
  await test('TC-PM04-02: check_duplicate_ingredient - Advil + Aleve dual NSAID toxicity detection', async () => {
    ...
    assert(res.success);
    const alert = res.data.find((a: any) => a.ingredient === 'NSAID Class');
  ...
  await test('TC-PM04-03: check_duplicate_ingredient - Lipitor + Atorvastatin brand/generic overlap', async () => {
    ...
    assert(res.success);
    const alert = res.data.find((a: any) => a.ingredient === 'Atorvastatin');
  ...
  await test('TC-PM05-01: suggest_schedule - shifts Atorvastatin from morning to bedtime', async () => {
    ...
    assert(res.success);
    const shift = res.data.proposedShifts.find((s: any) => s.medName === 'Atorvastatin');
  ...
  await test('TC-PM05-02: suggest_schedule - shifts Furosemide away from bedtime to morning', async () => {
    ...
    assert(res.success);
    const shift = res.data.proposedShifts.find((s: any) => s.medName === 'Furosemide');
    assert(!!shift);
  ...
  await test('TC-PM05-03: suggest_schedule - separates Calcium Carbonate from morning Levothyroxine', async () => {
    ...
    assert(res.success);
    const shift = res.data.proposedShifts.find((s: any) => s.medName === 'Calcium Carbonate');
    assert(!!shift);
