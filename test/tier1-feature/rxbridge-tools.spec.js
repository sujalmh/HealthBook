...
export async function runRxBridgeToolsTests() {
    let passed = 0;
    let failed = 0;
    const errors = [];
    ...
    // --- Tool 5: export_patient_summary (5 tests) ---
    await test('TC-RB05-01: export_patient_summary - generates 1-page summary with what changed and schedule', async () => {
        ...
        assert(res.success);
        assert(res.data.whatChangedSummary.length >= 3);
        assert(res.data.foodAndDietRules.length >= 1);
    });
